...
clearvars -except ifile i cell_file beh_file ses_num aligned_file thr_pass neuron_drop rev_trial_anova_win ...
    trial_idx_rec_set neuron_num_set fig_dir fig_dir_str
...
for icell = [1 4]
    ...
    clearvars trace_not_aligned_Rw_R trace_not_aligned_Rw_0 trace_not_aligned_Pn_P trace_not_aligned_Pn_0 trace_not_aligned_0_0
    trace_not_aligned_Rw_R = (trial_trace_out(Ident==2&Prob==75&Reward==1&temp_epoch,:));
    trace_not_aligned_Rw_0 = (trial_trace_out(Ident==2&Prob==75&Reward==0&temp_epoch,:));
    trace_not_aligned_Pn_P = (trial_trace_out(Ident==3&Prob==75&Reward==1&temp_epoch,:));
    trace_not_aligned_Pn_0 = (trial_trace_out(Ident==3&Prob==75&Reward==0&temp_epoch,:));
    trace_not_aligned_0_0 = (trial_trace_out(Prob==0&temp_epoch,:));
    
    trace_not_aligned_Rw_R_cue = (trial_trace_cue(Ident==2&Prob==75&temp_epoch,:));
    trace_not_aligned_Pn_P_cue = (trial_trace_cue(Ident==3&Prob==75&temp_epoch,:));
    trace_not_aligned_0_0_cue = (trial_trace_cue(Prob==0&temp_epoch,:));
    
    %%
    temp_trace_set_cue = {trace_not_aligned_Rw_R_cue [] trace_not_aligned_Pn_P_cue [] trace_not_aligned_0_0_cue};
    temp_trace_set_out = {trace_not_aligned_Rw_R, trace_not_aligned_Rw_0, trace_not_aligned_Pn_P, trace_not_aligned_Pn_0, trace_not_aligned_0_0};
    ...
    for ifill = [1 3 5]
        temp_fill = temp_trace_set_cue{ifill};
    ...
    for ifill = 1:5
        temp_fill = temp_trace_set_out{ifill};
        fill_y=[mean(temp_fill)+std(temp_fill,0,1)/sqrt(size(temp_fill,1)) ...
            fliplr(mean(temp_fill)-std(temp_fill,0,1)/sqrt(size(temp_fill,1)))];
        fill_hist = fill(fill_x,fill_y,cmap1(ifill,:));
        fill_hist.EdgeColor = 'none';
