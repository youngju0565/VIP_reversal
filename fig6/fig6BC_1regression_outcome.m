...
for i=1:ses_num
    ...
    Cue_1 = Cue_Rw;
    Cue_2 = Cue_Pn;
    Cue_Rw_1 = [0; Cue_1(1:end-1)];
    Cue_Pn_1 = [0; Cue_2(1:end-1)];
    Reward_1 = [0; Reward(1:end-1)];
    Punish_1 = [0; Punish(1:end-1)];
    Lick = zeros(size(Reward));
    Speed = zeros(size(Reward));
    GLM_T = table(Cue_1, Cue_2, Reward, Punish, Lick, Speed, Cue_Rw_1, Cue_Pn_1, Reward_1, Punish_1);
    ...
    fig_dir = ['E:\snl Dropbox\Jee Hyun\Ca imaging\Recording_data\figs\3_cue_RP\',...
        animal_name,'\',animal_name,'_',session_date,'\regression sliding revonset outcome term' win_size_str step_str hz_cut_str trial_str ' 230627\'];
    mkdir(fig_dir);
    ...
    reg_start = tic;
    % storage
    P_value_rwd = zeros(neuron_num,x_var_num,min_step_num);
    SRC_rwd = zeros(neuron_num,x_var_num,min_step_num);
    T_value_rwd = zeros(neuron_num,x_var_num,min_step_num);
    
    P_value_pun = zeros(neuron_num,x_var_num,min_step_num);
    SRC_pun = zeros(neuron_num,x_var_num,min_step_num);
    T_value_pun = zeros(neuron_num,x_var_num,min_step_num);
    ...
    for icell3 = 1:neuron_num
        ...
        for istep = 1:min_step_num
            ...
            % FON
            P_value_rwd(icell3,:,istep) = tmp_mdl_rwd.Coefficients.pValue(2:end);
            % SRC
            std_x_rwd = std(tmp_T_rwd{:,1:end-1})';
            std_y_rwd = std(tmp_T_rwd.Y);
            SRC_rwd(icell3,:,istep) = tmp_mdl_rwd.Coefficients.Estimate(2:end).*std_x_rwd/std_y_rwd;
            % t-value
            T_value_rwd(icell3,:,istep) = tmp_mdl_rwd.Coefficients.tStat(2:end);
            ...
            % FON
            P_value_pun(icell3,:,istep) = tmp_mdl_pun.Coefficients.pValue(2:end);
            % SRC
            std_x_pun = std(tmp_T_pun{:,1:end-1})';
            std_y_pun = std(tmp_T_pun.Y);
            SRC_pun(icell3,:,istep) = tmp_mdl_pun.Coefficients.Estimate(2:end).*std_x_pun/std_y_pun;
            % t-value
            T_value_pun(icell3,:,istep) = tmp_mdl_pun.Coefficients.tStat(2:end);
...
%%
warning on
unify_dir = ['E:\snl Dropbox\Jee Hyun\Ca imaging\Recording_data\figs\3_cue_RP\reversal_set regression outcome term sliding revonset'...
