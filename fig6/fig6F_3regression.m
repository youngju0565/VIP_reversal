...
%% Multiple linear regression with PR
%%
...
for i=1:ses_num
    ...
    animal_name = strsplit(beh_file{i},'_');
    animal_name = animal_name{1};
    anccr_dir = ['E:\snl Dropbox\Jee Hyun\Ca imaging\Mat_code\ANCCR\fit VIP ' fit_name '\' animal_name '\'];
    load([anccr_dir 'variables_' num2str(fit_var_num) 'var.mat'])
    
    varname_set = {'ANCCR_data','NC_data','SRC_data','PRC_data','PR_data'};
    varname_set2 = {'ANCCR','NC','SRC','PRC','PR'};
    
    for ivar = 1:length(varname_set)
        eval(sprintf("%s = cellfun(@(x) x(revses_idx),%s,'uniformoutput',false);",varname_set{ivar},varname_set{ivar}));
        eval(sprintf("%s = cellfun(@(x) x(trial_idx_rec),%s,'uniformoutput',false);",varname_set{ivar},varname_set{ivar}));
        eval(sprintf("%s_csrw_rw = %s{1};",varname_set2{ivar},varname_set{ivar}));
        eval(sprintf("%s_csrw_pn = %s{2};",varname_set2{ivar},varname_set{ivar}));
        eval(sprintf("%s_cspn_rw = %s{3};",varname_set2{ivar},varname_set{ivar}));
        eval(sprintf("%s_cspn_pn = %s{4};",varname_set2{ivar},varname_set{ivar}));
    end
    ...
    idenrev_trial_rec = find(trial_idx_rec>=idenrev_trial,1);
    for ivar = 1:length(varname_set)
        eval(sprintf("temp_csrw = %s_csrw_rw;",varname_set2{ivar}));
        eval(sprintf("temp_cspn = %s_cspn_rw;",varname_set2{ivar}));
        eval(sprintf("%s_csrw_rw(idenrev_trial_rec:end) = temp_cspn(idenrev_trial_rec:end);",varname_set2{ivar}));
        eval(sprintf("%s_cspn_rw(idenrev_trial_rec:end) = temp_csrw(idenrev_trial_rec:end);",varname_set2{ivar}));
        eval(sprintf("temp_csrw = %s_csrw_pn;",varname_set2{ivar}));
        eval(sprintf("temp_cspn = %s_cspn_pn;",varname_set2{ivar}));
        eval(sprintf("%s_csrw_pn(idenrev_trial_rec:end) = temp_cspn(idenrev_trial_rec:end);",varname_set2{ivar}));
        eval(sprintf("%s_cspn_pn(idenrev_trial_rec:end) = temp_csrw(idenrev_trial_rec:end);",varname_set2{ivar}));
    end
    ...
    fig_dir = ['E:\snl Dropbox\Jee Hyun\Ca imaging\Recording_data\figs\3_cue_RP\',...
        animal_name,'\',animal_name,'_',session_date,'\regression sliding revonset outcome ANCCR ' fit_name win_size_str step_str hz_cut_str trial_str ' 230627\'];
    mkdir(fig_dir);
    
    %% Regression
    fprintf("fitlm start: sliding\n"); warning off;
    reg_start = tic;
    x_var_num = size(GLM_T,2)+12;
    % storage
    ...
    for icell3 = 1:neuron_num
        ...
        for istep = 1:min_step_num
            ...
            temp_epoch((temp_first_trial:temp_first_trial+win_size_sliding-1)+(step_size)*(istep-1)) = true;
                ...
                % PR
                ivar = 5;
                eval(sprintf("tmp_T.%s_csrw_rw = %s_csrw_rw(temp_epoch);",varname_set2{ivar},varname_set2{ivar}));
                eval(sprintf("tmp_T.%s_csrw_pn = %s_csrw_pn(temp_epoch);",varname_set2{ivar},varname_set2{ivar}));
                eval(sprintf("tmp_T.%s_cspn_rw = %s_cspn_rw(temp_epoch);",varname_set2{ivar},varname_set2{ivar}));
                eval(sprintf("tmp_T.%s_cspn_pn = %s_cspn_pn(temp_epoch);",varname_set2{ivar},varname_set2{ivar}));
                
                temp_var_num = size(tmp_T,2);
                ...
                % FON
                P_value(icell3,1:temp_var_num,istep) = tmp_mdl.Coefficients.pValue(2:end);
                % SRC
                std_x = std(tmp_T{:,1:end-1})';
                std_y = std(tmp_T.Y);
                SRC(icell3,1:temp_var_num,istep) = tmp_mdl.Coefficients.Estimate(2:end).*std_x/std_y;
                % t-value
                T_value(icell3,1:temp_var_num,istep) = tmp_mdl.Coefficients.tStat(2:end);
        end
        toc
    end
...
%%
warning on
unify_dir = ['E:\snl Dropbox\Jee Hyun\Ca imaging\Mat_code\ANCCR\fit VIP ' fit_name '\reversal_set regression outcome ANCCR ' num2str(fit_var_num) 'var sliding revonset'...
