...
for i=1:ses_num
    ...
    fig_dir = ['E:\snl Dropbox\Jee Hyun\Ca imaging\Recording_data\figs\3_cue_RP\',...
        animal_name,'\',animal_name,'_',session_date,'\regression epochs outcome term' win_size_str hz_cut_str trial_str ' 230627\'];
    mkdir(fig_dir);
    ...
    reg_start = tic;
    % storage
    CPD_rwd = zeros(neuron_num,x_var_num,3);
    P_value_rwd = zeros(neuron_num,x_var_num,3);
    SRC_rwd = zeros(neuron_num,x_var_num,3);
    T_value_rwd = zeros(neuron_num,x_var_num,3);
    
    CPD_pun = zeros(neuron_num,x_var_num,3);
    P_value_pun = zeros(neuron_num,x_var_num,3);
    SRC_pun = zeros(neuron_num,x_var_num,3);
    T_value_pun = zeros(neuron_num,x_var_num,3);
    ...
    for iepoch = 1:3
        ...
        for icell3 = 1:neuron_num
            ...
            % FON
            P_value_rwd(icell3,:,iepoch) = tmp_mdl_rwd.Coefficients.pValue(2:end);
            % SRC
            std_x_rwd = std(tmp_T_rwd{:,1:end-1})';
            std_y_rwd = std(tmp_T_rwd.Y);
            SRC_rwd(icell3,:,iepoch) = tmp_mdl_rwd.Coefficients.Estimate(2:end).*std_x_rwd/std_y_rwd;
            % t-value
            T_value_rwd(icell3,:,iepoch) = tmp_mdl_rwd.Coefficients.tStat(2:end);
            ...
            for ivar = 1:x_var_num
                ...
                CPD_rwd(icell3,ivar,iepoch) = (var_SSE - tmp_SSE)/var_SSE;
            end
            ...
            % FON
            P_value_pun(icell3,:,iepoch) = tmp_mdl_pun.Coefficients.pValue(2:end);
            % SRC
            std_x_pun = std(tmp_T_pun{:,1:end-1})';
            std_y_pun = std(tmp_T_pun.Y);
            SRC_pun(icell3,:,iepoch) = tmp_mdl_pun.Coefficients.Estimate(2:end).*std_x_pun/std_y_pun;
            % t-value
            T_value_pun(icell3,:,iepoch) = tmp_mdl_pun.Coefficients.tStat(2:end);
            ...
            for ivar = 1:x_var_num
                ...
                tmp_mdl_var = fitlm(tmp_T_var);
                var_SSE = tmp_mdl_var.SSE;
                CPD_pun(icell3,ivar,iepoch) = (var_SSE - tmp_SSE)/var_SSE;
            end
    ...
    save([fig_dir, '\variables','_win_',num2str(win_size),'.mat'],...
        'tmp_mdl_rwd','tmp_mdl_pun','tmp_mdl_var','GLM_T','lick_out_post','speed_out_post','glm_y','neuron_num','iter_num','win_size')
    save([fig_dir, '\CPD','_win_',num2str(win_size),'_draw_ready.mat'],...
        'CPD_rwd','CPD_pun','neuron_num','win_size')
...
%%
warning on
unify_dir = ['E:\snl Dropbox\Jee Hyun\Ca imaging\Recording_data\figs\3_cue_RP\reversal_set regression outcome term'...
